    ...
    implements java.io.Serializable
{
  ...
  private static IFn debugFn;
  static {
    IFn require = Clojure.var("clojure.core","require");
    require.invoke(Clojure.read("com.gfredericks.test.chuck.regexes-test"));
    debugFn = Clojure.var("com.gfredericks.test.chuck.regexes-test","debug");
  }

  private void debug(String msg){
    debugFn.invoke(msg, temp, cursor);
  }
    ...
    public String getNormalizedPattern(){return normalizedPattern;}
...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    private CharProperty clazz(boolean consume) {
        ...
        int ch = next();
        for (;;) {
          debug("main loop, prev="+(prev == null ? "null" : prev.inspect())+", node="+(node == null ? "null" : node.inspect()));
            switch (ch) {
                case '^':
                ...
                case '&':
                    ...
                    if (ch == '&') {
                        ...
                        debug("end while("+(rightNode == null)+","+(prev==null)+")");
                        if (rightNode != null){
                            debug("node = rightNode;");
                            node = rightNode;
                        }
                        if (prev == null) {
                            if (rightNode == null)
                                ...
                                debug("prev = rightNode;");
                                prev = rightNode;
                            }
                        } else {
                            debug("prev = intersection(prev, node);");
    ...
    static final class SingleS extends CharProperty {
        String inspect(){return "SingleS["+((char) c)+"]";}
        final int c;
    ...
    static final class Single extends BmpCharProperty {
        String inspect(){return "Single["+((char) c)+"]";}
        final int c;
    ...
    static final class SingleI extends BmpCharProperty {
        String inspect(){return "SingleI["+((char) lower)+","+((char) upper) +"]";}
        final int lower;
        final int upper;
    ...
    private static CharProperty rangeFor(final int lower,
                                         final int upper) {
        return new CharProperty() {
                String inspect(){return "Range["+((char)lower)+","+((char)upper)+"]";}
                boolean isSatisfiedBy(int ch) {
    ...
    private static CharProperty union(final CharProperty lhs,
        ...
        return new CharProperty() {
          String inspect(){return "Union["+lhs.inspect()+","+rhs.inspect()+"]";}
                boolean isSatisfiedBy(int ch) {
    ...
    private static CharProperty intersection(final CharProperty lhs,
                                             final CharProperty rhs) {
        return new CharProperty() {
          String inspect(){return "Intersection["+lhs.inspect()+","+rhs.inspect()+"]";}
                boolean isSatisfiedBy(int ch) {
                    return lhs.isSatisfiedBy(ch) && rhs.isSatisfiedBy(ch);}};
    }
